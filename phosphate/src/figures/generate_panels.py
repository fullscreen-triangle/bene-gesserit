#!/usr/bin/env python3
"""
Generate individual panel figures (1×4 charts each) for the BPS paper series.
One panel per validation claim, saved to phosphate/docs/{paper}/figures/{id}.png

36 panels total: 6 papers × 6 panels each.
"""
...
_DOCS = os.path.join(os.path.dirname(os.path.dirname(_HERE)), 'docs')

DIRS = {
    'bs': os.path.join(_DOCS, 'bilayer-substrate',  'figures'),
    'se': os.path.join(_DOCS, 'state-encoder',      'figures'),
    'aa': os.path.join(_DOCS, 'aperture-array',     'figures'),
    'et': os.path.join(_DOCS, 'energy-transducer',  'figures'),
    'cf': os.path.join(_DOCS, 'cascade-fabric',     'figures'),
    'ri': os.path.join(_DOCS, 'readout-interface',  'figures'),
}
...
# ── Helpers ───────────────────────────────────────────────────────────────────
def _panel():
    fig = plt.figure(figsize=(18, 4.5), facecolor='white')
    gs  = gridspec.GridSpec(1, 4, figure=fig, wspace=0.36,
                            left=0.07, right=0.97, top=0.92, bottom=0.16)
    return fig, gs


def _a2(fig, gs, c):
    return fig.add_subplot(gs[0, c])
...
def _sv(fig, key, name):
    path = os.path.join(DIRS[key], name + '.png')
    fig.savefig(path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    print(f'  {key}/{name}.png')
...
def _bs_setup():
    ...
    def Vc(n): return 27.4 + 26.9 * n
    def lc(n): return 1.5 + 1.265 * n
    lh = 5.74; AL = 64.2
    Dhh = (2 * Vc(nc) / AL + 2 * lh) / 10
    return nc, Vc, lc, lh, AL, Dhh


def bs_01():
    nc, Vc, lc, lh, AL, Dhh = _bs_setup()
    fig, gs = _panel()

    a = _a2(fig, gs, 0)
    a.plot(nc, Vc(nc), 'o-', color=C[0])
    a.set(xlabel='nc', ylabel='Vc (Å³)')

    a = _a2(fig, gs, 1)
    a.plot(nc, lc(nc), 's-', color=C[1])
    a.set(xlabel='nc', ylabel='lc (Å)')
    ...
    _surf(a, ncm, lhm, (2 * Vc(ncm) / AL + 2 * lhm) / 10, cmap='viridis')
...
def bs_02():
    ...
    a.axhline(AL, ls='--', lw=0.8, color='#999')
    ...
    a = _a2(fig, gs, 2)
    a.plot(nc, Vc(nc) / (AL * lc(nc)), 'D-', color=C[3])
...
def bs_06():
    ...
    a = _a2(fig, gs, 0)
    a.plot(Vc_r, Vc_r / (AL * lc16), '-', color=C[0])
    a.axhspan(0.5, 1.0, alpha=0.12, color='green')
    a.set(xlabel='Vc (Å³)', ylabel='CPP')

    a = _a2(fig, gs, 1)
    a.plot(a0_r, Vc16 / (a0_r * lc16), '-', color=C[1])
    a.axhspan(0.5, 1.0, alpha=0.12, color='green')
    a.set(xlabel='a₀ (Å²)', ylabel='CPP')

    a = _a2(fig, gs, 2)
    a.plot(lc_r, Vc16 / (AL * lc_r), '-', color=C[2])
    a.axhspan(0.5, 1.0, alpha=0.12, color='green')
    a.set(xlabel='lc (Å)', ylabel='CPP')
...
def se_01():
    ...
    a = _a2(fig, gs, 0)
    a.bar(K_vals, [1.0] * len(K_vals), color=C[0], alpha=0.8, width=2)
    ...
    H_norm /= np.log(2)
    a = _a2(fig, gs, 2)
    a.plot(H_norm, H_norm, '-', color=C[2])
...
def se_03():
    ...
    gkgt = np.clip(1 / (sk3m * (1 - sk3m)) * 1 / (st3m * (1 - st3m)), 0, 2000)
...
def se_04():
    ...
    a = _a2(fig, gs, 2)
    for xy, col, lab in zip(NIST, NIST_COLORS, NIST_NAMES):
    ...
    a = _a3(fig, gs, 3)
    for xy, col in zip(NIST, NIST_COLORS):
...
def aa_01():
    ...
    W3 = np.clip((e**2) / (8 * pi * eps0 * rm3) * (1 / el3 - 1 / eps_w) / kBT, 0, 400)
...
def aa_02():
    ...
    mc_arr = float(np.mean(np.any(rng.random((50000, N_test)) < kappas, axis=1)))
    ...
    a = _a2(fig, gs, 1)
    a.bar(['Formula', 'Monte Carlo'], [formula_arr, mc_arr],
          color=[C[0], C[1]], alpha=0.85, width=0.4)
...
def aa_05():
    ...
    d_test = np.array([0, 0.1, 0.2, 0.5, 1.0, 2.0]); sig_t = 0.15
    a = _a2(fig, gs, 1)
    a.bar(range(len(d_test)), np.exp(-d_test**2 / (2 * sig_t**2)),
          color=C[2], alpha=0.85, width=0.6)
    a.set_xticks(range(len(d_test))); a.set_xticklabels([str(d) for d in d_test])
    a.set(xlabel='distance', ylabel='k (σ=0.15)')

    sig_arr = np.linspace(0.05, 0.5, 50)
    a = _a2(fig, gs, 2)
    a.plot(sig_arr, sig_arr * np.sqrt(2 * np.log(2)), '-', color=C[3])
...
def et_01():
    ...
    _surf(a, T3, n3, n3 * kB * T3 * ln2 * 1e21, cmap='plasma')
...
def et_04():
    ratio_Na = np.linspace(5, 25, 80)
    Vm_arr = np.linspace(-100e-3, 50e-3, 80)
    ...
    a = _a2(fig, gs, 1)
    a.plot(Vm_arr * 1e3, e * Vm_arr / kBT, '-', color=C[1])
...
def cf_01():
    ...
    fb3 = np.clip(b3**c3 / (c3 * np.log(b3)), 0, 30)
...
def cf_03():
    ...
    a = _a2(fig, gs, 2)
    a.fill_between(nc_v, [p**1 for p in P_test], [p**3 for p in P_test], alpha=0.25, color=C[2])
...
def ri_01():
    ...
    dQ3 = np.clip(b3q**(-k3q / 3) / 2, 1e-8, 1)
...
def ri_06():
    ...
    for lbl, cen in enumerate(centroids):
        samp = rng3.normal(cen, noise, (n_pc, 3)); samp = np.clip(samp, 0, 1)
        for s in samp:
...
def main():
    print('\n=== Generating BPS panel figures ===\n')
    panels = [
        ('bilayer_substrate', [bs_01, bs_02, bs_03, bs_04, bs_05, bs_06]),
        ('state_encoder',     [se_01, se_02, se_03, se_04, se_05, se_06]),
        ('aperture_array',    [aa_01, aa_02, aa_03, aa_04, aa_05, aa_06]),
        ('energy_transducer', [et_01, et_02, et_03, et_04, et_05, et_06]),
        ('cascade_fabric',    [cf_01, cf_02, cf_03, cf_04, cf_05, cf_06]),
        ('readout_interface', [ri_01, ri_02, ri_03, ri_04, ri_05, ri_06]),
    ]
    total = 0
    for paper, fns in panels:
        print(f'Paper: {paper}')
        for fn in fns:
            fn()
            total += 1
    print(f'\nDone: {total} panels written.')
